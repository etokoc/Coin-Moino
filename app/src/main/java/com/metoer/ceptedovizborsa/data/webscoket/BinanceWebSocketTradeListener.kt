...
class BinanceWebSocketTradeListener : WebSocketListener() {

    companion object {
        var data: MutableLiveData<CoinTradeData?>? = MutableLiveData()
    }
    ...
    override fun onOpen(webSocket: WebSocket, response: Response) {
         ...
         */
//        webSocket.send("")
        Log.i("WEBSOCKET BinanceWebSocketTradeListener", "Websockete bağlandı ")
    }
    ...
    override fun onMessage(webSocket: WebSocket, text: String) {
        ...
        if (text.isNotEmpty()) {
            ...
            data?.postValue(Gson().fromJson(json, CoinTradeData::class.java))
        } else {
    ...
    private fun writeToLog(text: String) {
        Log.i("WEBSOCKET BinanceWebSocketTradeListener", text)
    }
